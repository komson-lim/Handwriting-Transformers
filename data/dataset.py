...
def get_transform(grayscale=False, convert=True):
    ...
    transform_list = []
    transform_list.append(transforms.Resize((32, 192)))
    if grayscale:
...
class TextDataset():
    ...
    def __getitem__(self, index):
        ...
        imgs = [np.array(self.IMG_DATA_AUTHOR[idx]['img'].convert('L'))
                for idx in random_idxs]
        labels = [self.IMG_DATA_AUTHOR[idx]['label'].encode()
                  for idx in random_idxs]
        ...
        item = {'simg': imgs_pad, 'swids': imgs_wids, 'img': real_img,
                'label': real_labels, 'img_path': 'img_path', 'idx': 'indexes', 'wcl': index}
...
class TextCollator(object):
    ...
    def __call__(self, batch):
        ...
        simgs = torch.stack([item['simg'] for item in batch], dim=0)
        ...
        item = {'img': imgs, 'img_path': img_path, 'idx': indexes,
                'simg': simgs, 'swids': swids, 'wcl': wcls}
